...
- 背景使用 macOS material。
- 整体圆角约 20 pt，贴近系统浮层。
- 卡片为主角：安静头部 + 细类型色条 + 白色正文，圆角 14 pt，无投影；不绘制默认选中态，最新信息由卡片顺序表达。
- 层级由位置、材质和边界表达，不使用投影。
...
- 头部（高约 44 pt）：顶部细色条按类型着色（见 §9 类型色），显示类型名 + 相对时间 + 类型图标。
...
- 卡片 hover：~120ms ease-out 的轻微上移，不整体放大。
- 卡片按下：100-140ms 的 0.98 scale 反馈。
- 键盘定位卡片滚入视野：~160ms ease-out，不绘制可见选中态。
...
- 仅用于卡片顶部细色条与小图标，正文与列表其余部分仍跟随系统语义色，不扩散成全局品牌色。
...
- 信息单元从 Clip Row（紧凑行）改为 Clip Card（当时为大色块头部 + 白色正文；后续已收敛为细色条 + 白色正文）。
- 键盘导航主方向从上下改为左右。
- 顶部新增 Pinboard 分段标签（剪贴板 / 已固定）。
- 新增按类型的卡片头部色（§9 类型色）；后续已收敛为细色条与图标。
...
- 跟随系统语义色：类型色只用于卡片细色条与图标，不扩散成全局品牌色；正文仍用系统色。
...
- (a) 的 Paste 式卡片墙不变：类型色仅用于内容卡片，不是玻璃，不与「玻璃只给控件层」冲突。
