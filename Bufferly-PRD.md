...
Bufferly 是一款面向普通 macOS 用户的本地优先剪贴板助手。
...
用户刚复制了短信验证码，随手又复制了别的内容导致验证码丢失。用户按快捷键呼出 Bufferly，验证码就在最前面，回车粘贴。
...
用户昨天复制过一个网页链接，今天想再发给朋友。呼出 Bufferly，搜索记得的关键词，链接自动归类为「链接」类型，一眼找到。

### 场景 4：找回复制过的图片和文件

用户复制过一张截图或一个文件，后来又复制了别的内容。呼出 Bufferly，图片有缩略图、文件有图标和名称，选中即可原样粘贴。

### 场景 5：敏感内容自动保护

用户复制密码、验证码或密钥。Bufferly 判断为敏感内容，默认不保存明文，或只保存脱敏预览，不会在历史里泄露。
